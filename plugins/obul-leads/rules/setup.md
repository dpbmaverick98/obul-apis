...
```sh
curl -s -X POST "https://proxy.obul.ai/proxy/https/stableenrich.dev/api/hunter/email-verifier" \
  -H "Content-Type: application/json" \
  -H "x-obul-api-key: $OBUL_API_KEY" \
  -d '{"email": "test@example.com"}' | head -c 200
```

A successful response returns JSON with verification results. If you see an authentication error, verify your key is correct.
